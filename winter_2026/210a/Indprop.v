...
Lemma Perm3_refl : forall (X : Type) (a b c : X),
  Perm3 [a;b;c] [a;b;c].
Proof.
  intros. apply perm3_trans with (l2 := [a;c;b]). 
  - apply perm3_swap23.
  - apply perm3_swap23.
Qed.
...
Proof.
  ...
  - discriminate.
  - destruct H. destruct H. injection H as H1. rewrite <- H1 in H0. apply ev_inversion in H0. 
  destruct H0. 
    + discriminate.
    + destruct H.  destruct H. injection H as H2. rewrite <- H2 in H0. apply H0. 
Qed. 
  (* FILL IN HERE Admitted. *)
(** [] *)
...
Proof.
  ...
  {
    intros. simpl. apply ev_SS. apply IHev. apply H0.
  }
Qed.
  (* FILL IN HERE Admitted. *)
(** [] *)
...
Proof.
  ...
  {
    intros. simpl in H. inversion H. apply IHev. apply H2.
  }
Qed.
...
Lemma or_comm_iff : forall P Q, P \/ Q <-> Q \/ P.
Proof. split; apply or_commut. Qed.
...
Proof.
  intros. induction H. 
  {
    simpl in H0. simpl. 
    apply or_assoc.
    rewrite (or_comm_iff (b = x) (a = x)). apply or_assoc in H0. apply H0.
    
  }
  {
    simpl in H0. simpl. 
    (* apply or_assoc. *)
    destruct H0. left;apply H. destruct H. right;right;left;apply H. destruct H. right;left;apply H. exfalso; apply H.     
  }
  {
...
Theorem n_le_m__Sn_le_Sm : forall n m,
  n <= m -> S n <= S m.
Proof.
intros n. induction n; intros. induction H. apply le_n. apply le_S. apply IHle. induction H. apply le_n. apply le_S. apply IHle.   
 Qed.
  (* FILL IN HERE Admitted. *)
...
Proof.
intros. inversion H. apply le_n. apply le_trans with (n:= S n). apply le_S;apply le_n. apply H1.
Qed.
  (* FILL IN HERE Admitted. *)
...
Proof.
  ...
  - apply le_S; apply IHle. 
Qed.  
  (* induction n1. simpl in H. apply O_le_n. inversion H. apply le_plus_l.  *)
 (* FILL IN HERE Admitted. *)
...
Proof. 
  ...
  - induction p. generalize dependent m. 
    +intros. rewrite add_comm in H. simpl in H. apply plus_le in H. destruct H. right. apply H.
...
(** **** Exercise: 3 stars, standard, optional especially useful (R_provability)
...
| subseq_nil (a:list nat): subseq [] a
| subseq_strict (a b :list nat) (x:nat): subseq a b -> subseq a (x :: b)
| subseq_peel (a b :list nat) (x:nat) : subseq a b -> subseq (x::a) (x :: b)
(* FILL IN HERE *)
.
...
Proof.
  ...
  - simpl. apply subseq_strict. apply IHsubseq.
  - simpl. apply subseq_peel. apply IHsubseq.
Qed.  
...
Proof.
  ...
  - inversion H. apply subseq_nil.
  - apply IHsubseq in H. apply subseq_strict. apply H. 
  - inversion H; try apply subseq_nil. 
    + apply subseq_strict. apply IHsubseq. apply H3.
    + apply subseq_peel. apply IHsubseq. apply  H3. 
Qed.
...
Proof.
  intros. split. 
  - intros. destruct H. induction H; try reflexivity. 
  {simpl. rewrite IHexp_match1. rewrite IHexp_match2. simpl. reflexivity. }
  {simpl. rewrite IHexp_match. simpl. reflexivity. }
  { simpl. rewrite IHexp_match. simpl. rewrite orb_true_iff. right. reflexivity. }
  - intros. induction re. 
  {discriminate. }
  { exists nil. apply MEmpty. }
  { exists [t]. apply MChar. }
  {simpl in H. rewrite andb_true_iff in H. destruct H. apply  IHre1 in H. apply  IHre2 in H0. destruct H. destruct H0. exists (x ++ x0). apply MApp. apply H. apply H0. }
...
Proof.
intros. simpl in H. inversion H. inversion H1. Qed.
  (* FILL IN HERE Admitted. *)
(** [] *)
...
Proof.
  ...
  destruct H. 
  - apply IH1 in H. destruct H. destruct H.  destruct H. destruct H.   exists x, x0, (x1 ++ s2). split.
    + rewrite app_assoc. rewrite app_assoc.  f_equal. rewrite <- app_assoc. apply H. 
    + split .
      { destruct H0. apply H0. }
      { destruct H0. intros. rewrite app_assoc. rewrite app_assoc. apply MApp. rewrite <- app_assoc. apply H1. apply Hmatch2. }
  - apply IH2 in H.  destruct H. destruct H.  destruct H. destruct H.  
  exists (s1 ++ x) , x0, (x1).
  split.
    + rewrite <-app_assoc. f_equal. apply H.
    + destruct H0. split.
      * apply H0. 
...
Proof.
  ...
  exists x, x0, x1. split. apply H. split. destruct H0. apply H0. destruct H0. intros. apply MUnionL. apply H1. 
Qed. 
  (* FILL IN HERE Admitted. *)
(** [] *)
...
Proof.
  ...
  apply IH in H. destruct H. destruct H. destruct H. destruct H.
  exists x, x0, x1. split. apply H. split. destruct H0. apply H0. destruct H0. intros. apply MUnionR. apply H1. 
Qed. 
...
Proof.
  ...
  - inversion H. split. intros. exfalso. apply H0. apply H1. 
...
Proof.
  ...
  - unfold not. intros. simpl in Hcount. destruct H. 
